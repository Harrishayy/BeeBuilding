  ...
  return (
    ...
    >
      ...
      <div className="pixel-border" style={{ padding: 12 }}>
        <div className="pixel-text" style={{ fontSize: 12, color: '#fff', marginBottom: 6 }}>
          {plan.title}
        </div>
        <div className="pixel-text" style={{ fontSize: 9, color: '#ccc', whiteSpace: 'pre-wrap' }}>
          {plan.summary}
        </div>
      </div>
      ...
      {/* Requirements */}
      <div className="pixel-border" style={{ padding: 12 }}>
        <div className="pixel-text" style={{ fontSize: 11, color: '#ffd54f', marginBottom: 6 }}>
          REQUIREMENTS
        </div>
        {plan.requirements.map((req, i) => (
          <div key={i} style={{ display: 'flex', gap: 6, marginBottom: 4 }}>
            <span className="pixel-text" style={{ fontSize: 9, color: '#4caf50' }}>{'\u2610'}</span>
            <span className="pixel-text" style={{ fontSize: 9 }}>{req}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        <div className="pixel-border" style={{ padding: 12, flex: 1 }}>
          ...
          {plan.risks.map((risk, i) => (
            <div key={i} className="pixel-text" style={{ fontSize: 9, color: '#f44336', marginBottom: 2 }}>
              {'\u26A0'} {risk}
            </div>
          ))}
        </div>
        <div className="pixel-border" style={{ padding: 12, minWidth: 80, textAlign: 'center' }}>
          ...
          <div className="pixel-text" style={{ fontSize: 14, color: complexityColor }}>
